...
-- See a window into what the freight was two orders ago (lag) or 1 order ahead (lead)
SELECT CustomerID, 
OrderDate,
Freight,
LAG(Freight, 2) OVER (PARTITION BY CustomerID ORDER BY OrderDate) as PrevOrder,
LEAD(Freight, 1) OVER (PARTITION BY CustomerID ORDER BY OrderDate) as FutureOrder
FROM Orders
ORDER BY CustomerID, OrderDate DESC